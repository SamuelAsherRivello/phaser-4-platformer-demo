...
- [x] 1.1 Size the Phaser `Scale.NONE` canvas to the active browser viewport at startup and on browser resize while keeping the loaded map as the world/camera bounds; configure the main camera to follow the existing player and recalculate a centered 50-percent-width by 50-percent-height deadzone on Phaser resize. Replace the obsolete map-sized-canvas source assertion and verify focused checks cover viewport sizing, follow, bounds, half-viewport geometry, and resize handling.
